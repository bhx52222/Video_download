...
- yt-dlp：https://github.com/yt-dlp/yt-dlp 。1.4.1 使用外部环境，1.5 独立包将其及相关依赖放入包内，按各组件许可使用。
- 1.5 内置 Python、FFmpeg、FunASR、ModelScope、PyTorch 和下载依赖；Mac 还包含 parakeet-mlx，Windows 包含 faster-whisper、RapidOCR / ONNX Runtime。Python 包的 dist-info / licenses 保留在运行时目录，Windows 下载来源与 SHA-256 写入 resources/provenance.json；模型使用各自许可，首次使用下载。公开正式发布前仍需完成完整依赖清单与对应源码资料核验。
