...
云端容器里只有裸 Python，没有 ffmpeg、yt-dlp、`~/.vx`。凡是断言涉及
"某个外部工具在不在"，在这里"找不到"的分支恒真，写出来必然通过，
到装了全套环境的 Mac 上必然失败。

涉及外部工具查找的测试，**自己把 PATH 指到临时空目录**再断言，
或者断言与机器状态无关的性质。`scripts/test_runtime_paths.py` 第 3 节
是踩过这个坑之后的写法。
