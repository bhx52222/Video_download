...
| 取消任务的信号掩码缺陷 | `runner.py` 在 `pthread_sigmask` 屏蔽 SIGTERM 期间 `Popen`，被阻塞的掩码被子进程继承（实测 `SigBlk=0000000000004002`），内核子进程对 SIGTERM 免疫，发完状态仍为 `S`。后果不是杀不死而是只能强杀：用户点取消白等 3 秒宽限期，yt-dlp/ffmpeg 无机会收尾，留下半截 `.part`。原有测试只验"最终停了"，SIGKILL 谁也挡不住，故长期未暴露。已在 `vx_process.spawn` 用 `preexec_fn` 清子进程掩码；`test_process_tree.py` 加了只发 SIGTERM、不给 SIGKILL 兜底的断言，撤掉修复即失败。 |
| 跨平台进程与路径 | 为 Windows 版铺地基。`vx_process.py` 抽出进程组创建与树终止（POSIX `start_new_session`+`killpg`，Windows `CREATE_NEW_PROCESS_GROUP`+`taskkill /T`）；`vx_runtime.py` 增加 Windows 布局（顶层 `python.exe`、`.exe` 后缀、`%LOCALAPPDATA%`）。平台规则做成纯函数参数，两平台规则均在 Linux 上覆盖；POSIX 进程树终止为真实起进程验证（含孙子进程不留孤儿）。macOS 行为未变，已有回归全过。Windows 分支未在真机执行。 |
